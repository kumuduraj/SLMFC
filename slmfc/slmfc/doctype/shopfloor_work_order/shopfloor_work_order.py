...
class ShopfloorWorkOrder(Document):
    ...
    def set_costs(self, se):
        ...
        tot_std = tot_act = 0.0
        for wo_row, se_row in zip(rows, consumed):
    ...
    def make_stock_entry(self):
        ...
        for i in self.items:
            ...
            if q <= 0:
                continue
            se.append("items", {
                "item_code": i.item_code, "qty": q, "uom": i.uom, "stock_uom": i.uom,
                "conversion_factor": 1, "transfer_qty": q, "s_warehouse": self.wip_warehouse,
            })
        se.append("items", {
            "item_code": self.item_code, "qty": flt(self.output_qty), "uom": stock_uom,
            "stock_uom": stock_uom, "conversion_factor": 1, "transfer_qty": flt(self.output_qty),
            "t_warehouse": self.target_warehouse, "is_finished_item": 1, "bom_no": self.bom,
        })
